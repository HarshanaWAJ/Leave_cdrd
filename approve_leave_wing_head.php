                        ...
                        if ($result === false) {
                            ...
                            while ($row = $result->fetch_assoc()) {
                                if (isset($_SESSION['officer_wing']) && $row['wing'] == $_SESSION['officer_wing']) {
                                    ...
                                    list($statusLabel, $statusColor) = getStatusInfo2($row['status1']);
                                
                                    

                                    // Check if the status is 'approve' to enable/disable the button
                                    $disabled = ($row['status'] === 'approve') ? '' : 'disabled';
            ...
            <section class="section">
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        // SQL query to fetch all columns from the 'leave_applications' table
                        $sql = "SELECT * FROM leave_applications_ero WHERE  (status1 IN ('pending', 'Unknown') OR status1 IS NULL OR status1 = '')";
            ...
            <section class="section">
                <div class="card">
                    <div class="card-body">
                        ...
                        // SQL query to fetch all columns from the 'leave_applications' table
                        $sql = "SELECT * FROM leave_applications WHERE (status1 = 'approve' ) AND (status2 IN ('pending', 'Unknown') OR status2 IS NULL OR status2 = '')";
                        ...
                        if ($result === false) {
                            ...
                            while ($row = $result->fetch_assoc()) {
                                if (isset($_SESSION['officer_wing']) && $row['wing'] == $_SESSION['officer_wing']) {
                                    ...
                                    // Get status label and color
                                    list($statusLabel, $statusColor) = getStatusInfo($row['status2']);
