                                ...
                                <div class="col-md-12">
                                    ...
                                    <table class='table' id="table1">
                                        ...
                                        <tbody id="leaveTableBody">
                                            ...
                                            // SQL query to fetch canceled leaves for interns
                                            $sqlCanceledInterns = "SELECT * FROM leave_applications 
                                                                WHERE status1 = 'decline' AND status2 = 'decline' AND status3 = 'decline'"; 
                                            $resultCanceledInterns = $conn->query($sqlCanceledInterns);

                                            // Display canceled leaves for interns
                                            while ($rowCanceledInterns = $resultCanceledInterns->fetch_assoc()) {
                                                echo '<tr>';
                                                echo '<td>' . $rowCanceledInterns['name'] . '</td>';
                                                echo '<td>' . $rowCanceledInterns['intern_id'] . '</td>';
                                                echo '<td>' . $rowCanceledInterns['leave_type'] . '</td>';
                                                echo '<td>' . $rowCanceledInterns['from_date'] . '</td>';
                                                echo '<td>' . $rowCanceledInterns['to_date'] . '</td>';
                                                echo '</tr>';
                                            }
                            ...
                            <div style='overflow-x: auto;'>
                                <table class='table' id="table1">
                                    ...
                                    <tbody id="leaveTableBody2">
                                        ...
                                        // SQL query to retrieve canceled leaves for officers
                                        $sqlCanceledOfficers = "SELECT * FROM leave_applications_officers 
                                                            WHERE status1 = 'decline' AND status2 = 'decline' AND status3 = 'decline'";
                                        ...
                                        if ($resultCanceledOfficers->num_rows > 0) {
                                            while ($rowCanceledOfficers = $resultCanceledOfficers->fetch_assoc()) {
                                                echo "<tr>";
                                                echo "<td>" . $rowCanceledOfficers['name'] . "</td>";
                                                echo "<td>" . $rowCanceledOfficers['officer_number'] . "</td>";
                                                echo "<td>" . $rowCanceledOfficers['leave_type'] . "</td>";
                                                echo "<td>" . $rowCanceledOfficers['from_date'] . "</td>";
                                                echo "<td>" . $rowCanceledOfficers['to_date'] . "</td>";
                                                echo "</tr>";
                                            }
                                        } else {
