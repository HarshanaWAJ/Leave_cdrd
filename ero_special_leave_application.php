...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (
        ...
        $externalId = $conn->real_escape_string($_POST["externalId"]);
        ...
        $sql = "INSERT INTO leave_applications_ero (name, leave_type, from_date, to_date, from_time, to_time, number_of_days, external_id, reason, wing, remarks)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        // Bind parameters
        $stmt->bind_param("sssssssssss", $name, $leaveType, $fromDate, $toDate, $fromTime, $toTime, $numberOfDays, $externalId, $reason, $wing, $remarks);
            ...
            echo "<script>
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        window.location.href = 'external_research_officer_apply_leave.php';
                    }
                });
            </script>";
        } else {
