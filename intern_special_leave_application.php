...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (
        ...
        $numberOfDays = $interval->days + 1; // Add 1 to include both start and end dates

        // Perform database insertion using prepared statements for security
        $sql = "INSERT INTO leave_applications (name, leave_type, from_date, to_date, from_time, to_time, number_of_days, intern_id, reason, wing, remarks)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        // Bind parameters
        $stmt->bind_param("sssssssssss", $name, $leaveType, $fromDate, $toDate, $fromTime, $toTime, $numberOfDays, $internId, $reason, $wing, $remarks);
            ...
            echo "<script>
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        window.location.href = 'intern_apply_leave.php';
                    }
                });
            </script>";
        } else {
