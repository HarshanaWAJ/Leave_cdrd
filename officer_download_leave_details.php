        ...
        <nav class="navbar navbar-header navbar-expand navbar-light">
            ...
            <div class="main-content container-fluid">
                ...
                <div class="col-4">
                    ...
                    <div class="col-4">
                        <label for="forcee">Force/Name:</label>
                        <fieldset class="form-group">
                            <select class="form-select" id="basicSelect"  name="forcee" required>
                                <option value="" disabled selected>Select Force/Name</option>
...
if (!empty($forcee)) {
    $sql .= " AND officers.force = '$forcee' OR officers.name = '$forcee'";
}
...
$result = $conn->query($sql);
                            ...
                            <button id="downloadOfficerBtn" class="btn btn-secondary" onclick="downloadOfficer()">Download Officer Leave Report</button>
                        </div>
                    </div>
                </section>
                    ...
                    <div class="card">
                        ...
                        <h3>Approved Leaves of External Research Officers</h3>
                            ...
                            <div class="row mb-3">
                                ...
                                <tbody id="leaveTableBody3">
                                <?php
                                    // Include the database connection file
                                    include 'db.php';

                                    // SQL query to retrieve all leaves from leave_applications table
                                    $sql = "SELECT * FROM leave_applications_ero WHERE from_date BETWEEN '$fromDate' AND '$toDate' AND status3 = 'approve' ";
                                    $result = $conn->query($sql);
                            ...
                            <button id="downloadEROBtn" class="btn btn-secondary" onclick="downloadERO()">Download Externsl Research Officer Leave Report</button>
                        </div>
                    </div>
                </section>
    ...
    <script>
        ...
        function downloadERO() {
            html2canvas(document.getElementById('table3')).then(function(canvas) {
                const imgData = canvas.toDataURL('image/png');
                const pdf = new jspdf.jsPDF('p', 'mm', 'a4');
                const imgProps = pdf.getImageProperties(imgData);
                const pdfWidth = pdf.internal.pageSize.getWidth();
                ...
                pdf.addImage(imgData, 'PNG', 0, 0, pdfWidth, pdfHeight);
                pdf.save('ERO_leaves_summary.pdf');
            });
        }
...
<script>
    // Function to fetch and update leave data based on the selected date range and force/name
    function updateLeaveData(fromDate, toDate, force) {
        ...
        // Make an AJAX request to fetch leave data based on the selected date range for interns
        var xhr1 = new XMLHttpRequest();
        xhr1.open('POST', 'get_ero_pdf.php', true);
        xhr1.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        xhr1.onreadystatechange = function () {
            if (xhr1.readyState === 4 && xhr1.status === 200) {
                // Update the leave table for interns with the retrieved data
                document.getElementById('leaveTableBody3').innerHTML = xhr1.responseText;
            }
        };
        xhr1.send('fromDate=' + fromDate + '&toDate=' + toDate);
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        searchBtn.addEventListener('click', function () {
            ...
            // Fetch and update leave data based on the selected date range and force/name
        ...
        // Initial check to disable search button if default values are empty
