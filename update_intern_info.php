...
if (isset($id)) {
    ...
    $stmt = $conn->prepare("SELECT * FROM `intern` WHERE intern_id = ?");
    ...
    $wing = '';
    $email = '';
    $username = '';  // Correct variable name
    $password = '';  // Correct variable name
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (
        ...
        $intern_id = (int)$_POST["intern_id"]; // Convert to integer
        ...
        // Perform database update using prepared statements for security
        $sql = "UPDATE intern SET intern_id=?, name=?, name_in_full=?, permanent_address=?, university=?, trade=?, wing=?, email=?, username=?, password=? WHERE intern_id=?";

        // Prepare the statement
        $stmt = $conn->prepare($sql);

        // Bind parameters
        $stmt->bind_param("isssssssssi", $intern_id, $name, $name_in_full, $permanent_address, $university, $trade, $wing, $email, $username, $hashedPassword, $intern_id);
            ...
            <div class="col-12 col-md-6 order-md-2 order-first">
                <nav aria-label="breadcrumb" class='breadcrumb-header'>
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Update Intern</li>
                    </ol>
                </nav>
            </div>
        </div>
                                    ...
                                    <div class="col-md-6 col-12">
                                        <div class="form-group has-icon-left">
                                            ...
                                            <div class="position-relative">
                                                <input type="text" class="form-control" placeholder="Intern ID" id="first-name-icon" name="intern_id" value="<?php echo htmlspecialchars($intern_id); ?>" required>
                                    ...
                                    <div>
                                        <div class="col-md-12 col-12">
                                            <div class="form-group has-icon-left">
                                                ...
                                                <div class="position-relative">
                                                    <input type="text" class="form-control" placeholder="University" id="first-name-icon" name="university" value="<?php echo htmlspecialchars($university); ?>" required>
                                                    <div class="form-control-icon">
        ...
        <script>
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = 'manage_interns.php';
                }
            });
        </script>
    <?php endif; ?>
