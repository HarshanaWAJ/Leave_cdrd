<?php
    ...
    $query = "SELECT name FROM officers WHERE position IN ('Deputy Director General', 'Cheif Coordinator')";
       ...
       AND ((from_date BETWEEN ? AND ?) OR (to_date BETWEEN ? AND ?))";
        ...
        if ($stmt = $conn->prepare($totalLeaveDaysQuery)) {
            ...
            if ($total_leave_days >= 3) {
                ...
                echo "<button class='btn btn-danger' onclick='window.location.href=\"cc_special_leave_application.php\"'>Apply for Special Leave</button>";
                echo "</div>";
            } else {
...
?>
                                        ...
                                        $query = "SELECT name, officer_number FROM officers WHERE position IN ('Deputy Director General', 'Cheif Coordinator')";
                                        $result = $conn->query($query);
