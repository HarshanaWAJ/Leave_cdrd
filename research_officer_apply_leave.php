                            ...
                            <div class="card">
                                <div class="card-content">
                                    ...
                                    <?php
                                        ...
                                        $session_officer_id = $_SESSION['officer_number'];
                                        ...
                                        if ($_SERVER["REQUEST_METHOD"] == "POST") {
                                            ...
                                            if (
                                                ...
                                                // Check if session officer ID is not equal to acting officer ID in the specified period
                                                $conflictQuery = "SELECT COUNT(*) FROM leave_applications_officers 
                                                                WHERE assigned_officer_id = ? 
                                                                AND ((from_date BETWEEN ? AND ?) OR (to_date BETWEEN ? AND ?))";

                                                if ($stmt = $conn->prepare($conflictQuery)) {
                                                    $stmt->bind_param("sssss", $session_officer_id, $fromDate, $toDate, $fromDate, $toDate);
                                                    $stmt->execute();
                                                    ...
                                                    if ($conflictCount > 0) {
                                                        ...
                                                        // Check the total number of leave days for the officer in the specified month
                                                        $totalLeaveDaysQuery = "SELECT SUM(number_of_days) AS total_leave_days 
                                                                                FROM leave_applications_officers 
                                                                                WHERE officer_number = ? 
                                                                                AND MONTH(from_date) = ?";

                                                        if ($stmt = $conn->prepare($totalLeaveDaysQuery)) {
                                                            $stmt->bind_param("si", $officer_number, $month); // "si" indicates one string parameter and one integer parameter
                                                            $stmt->execute();
                                                            $stmt->bind_result($total_leave_days);
                                                            $stmt->fetch();
                                                            $stmt->close();
                                                            ...
                                                            if ($total_leave_days >= 3) {
                                                                ...
                                                                echo "<button class='btn btn-danger' onclick='window.location.href=\"research_officer_special_leave_application.php\"'>Apply for Special Leave</button>";
                                                                echo "</div>";
                                                            } else {
                                                                ...
                                                                if (is_numeric($numberOfDays)) {
                                                                    ...
                                                                    $remainingDays = 3 - $total_leave_days;
                                                                    if ($numberOfDays > $remainingDays) {
                                                                        echo "<div class='error-message' style='color: red;'>";
                                                                        echo "You cannot apply for more than $remainingDays days of leave.";
                                                                        echo "If you have special reason click below button to apply<br><br>";
                                                                        echo "<button class='btn btn-danger' onclick='window.location.href=\"research_officer_special_leave_application.php\"'>Apply for Special Leave</button>";
                                                                    } else {
                                                                        // Perform database insertion using prepared statements for security
                                                                        $sql = "INSERT INTO leave_applications_officers (name, leave_type, from_date, to_date, from_time, to_time, number_of_days, officer_number, wing, position, reason, assigned_officer_id, assigned_officer_name)
                                                                                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

                                                                        // Prepare the statement
                                                                        if ($stmt = $conn->prepare($sql)) {
                                                                            // Bind parameters
                                                                            $stmt->bind_param("sssssssssssss", $name, $leaveType, $fromDate, $toDate, $fromTime, $toTime, $numberOfDays, $officer_number, $officerwing, $position, $reason, $assigned_officer_id, $assigned_officer_name);
